...
class DeTempleImprovedTests(unittest.TestCase):
    def test_improved_construction_is_a_verified_29_e_upper_bound(self):
        report = verify_files(IMPROVED_DIRECTORY / "construction.json", PROFILE_PATH)

        self.assertTrue(report.valid)
        self.assertEqual(report.data["score"], {"metric": "e_move", "e_move": 29})
        self.assertEqual(
            report.data["draw_operations"],
            {"lines": 10, "circles": 19, "total": 29},
        )
        self.assertEqual(report.data["targets"], ["B_plus", "B_minus"])
        self.assertEqual(report.data["first_target_e_move"], 29)
        ...
        self.assertEqual(baseline_score - report.data["score"]["e_move"], 3)
    ...
    def test_dependency_graph_matches_certificate_program(self):
        ...
        self.assertEqual(graph["total_e_move"], 29)

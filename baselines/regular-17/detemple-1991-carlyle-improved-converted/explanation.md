# DeTemple 1991 修改版 Carlyle 圆构造的 24 E 转写

## 结论与证据等级

本目录把 DeTemple 论文第 104 页明确给出的两项修改转写为
`regular-17-e-fixed-v1`，删除被半尺度路线完全取代的完整尺度根分支，并用
垂直平分目标弦替换最后一次单位长度搬运。
...
因此 24 E 是同一 profile 下比原 32 E baseline 短 8 E 的新已验证上界。它不是
...
可折叠圆规基础操作。因此原文修改版的 45 与这里的 24 E 不可直接比较。
...
\[
H_{0,8}=(2\cos(2\pi/17),0).
\]

原转写把单位长度搬运到 (H_{0,8})，再以它为圆心画单位圆，需要 6 E。这里改为
画以 (O)、(H_{0,8}) 为圆心且互相经过对方的两个圆，再连接两圆交点。所得直线
是 (OH_{0,8}) 的中垂线，即
...
它与初始单位圆的两个交点就是 (B_+)、(B_-)。该末段只需 2 个圆和 1 条直线，
由 6 E 降为 3 E。
...
第一次距离搬运仍按 Euclid I.2 风格完全展开。交点绑定为 0 E，但所有用于画线、
画圆的点都可追溯到两个已构造对象的确定性交点。最终不再进行距离搬运。

...
因此 21 E 是同一 profile 下比原 32 E baseline 短 11 E 的新已验证上界。它不是
...
可折叠圆规基础操作。因此原文修改版的 45 与这里的 21 E 不可直接比较。
...
因此只需 1 E 即可直接得到 (Y)。由于旧搬运的末圆原本还会复用于 (OY) 的
中垂线，新路线需要补画以 (O)、(Y) 为圆心且互相经过对方的两个圆。连同两条
中垂/连接直线和最后的 Carlyle 圆，这一组合段由 9 E 降为 6 E。
...
检查拓扑顺序、引用一致性、构造哈希、总分，以及所有 21 个计费节点均属于
最终目标弦的祖先链。

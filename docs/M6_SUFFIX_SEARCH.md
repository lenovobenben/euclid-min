...
> 历史说明：本文记录 19 E 尚为项目上界时的搜索。当前上界已于
> 2026-09-05 更新为 17 E，本文的未命中结论仅适用于其中冻结的前缀。
...
该轮没有生成候选证书；在该轮结束时，已验证上界仍为 19 E。按照预先约定的
停止条件，至此结束通用启发式扩宽路线；上述所有未命中结果都不能升级为
19 E 最优性证明。

### 8.3 固定 E16 状态的精确两步穷尽

后续几何—代数 IR 工作取消了 8.2 节的浮点 shortlist 限制。对同一个固定 E16
状态，精确枚举了全部 22,454 个不同首步对象，并利用“末笔必须使用至少一个首步
新交点”的安全归约检查 202,855,848 个末笔参数化。结果为 0 命中、0 未决关系。

这把 8.2 节的启发式负结果升级为该**固定状态**上的完备两步排除：现有前 16E
不能只靠改写后两笔形成 18E。它仍不约束其他 16E 前缀，因而不是全局下界。
机器可读结果见
[`e16-two-step-target-extension-v2-sage-10.7.json`](../benchmarks/e16-two-step-target-extension-v2-sage-10.7.json)。

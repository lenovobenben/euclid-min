...
当前新的已验证上界为 24 E，原 32 E 条目作为首个 baseline 保留。系统文献检索尚未完成，因此不能把 24 E 称为文献最短或最短已知。
...
| `detemple-1991-carlyle-improved-converted` | DeTemple, 1991, §4, p. 104, both stated modifications; superseded branch removed; final target chord used | `converted` | 24 E | Sage verifier 通过；新上界 |

原文从单位圆和两条坐标轴开始，使用 modern non-collapsing compass，并报告未经修改构造的 Lemoine simplicity 为 51、修改版为 45。本项目两条转写均为坐标轴计费：未经修改路线并展开两次距离搬运后为 32 E；应用原文两项修改、删除已被半尺度路线取代的完整尺度根分支，并用垂直平分目标弦替换最后一次距离搬运后为 24 E。原文 51/45 与本项目的 32/24 E 只能并列记录，不能跨 metric 直接比较；本项目内部的 32 E 与 24 E 使用同一 profile，可以直接比较。
...
当前项目满足“比已验证 baseline 更短”的门槛，并可把 24 E 作为同一 profile 下后续结果的比较上界；尚不满足“已审阅文献中的最短已知”或“全局最优”的声明门槛。

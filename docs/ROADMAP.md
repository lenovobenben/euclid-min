...
- 该结果把当前上界更新为 17 E，不包含 0–16 E 的完备下界证明。

## M7：Proof Mode

> 状态：**待完成（当前暂停）**。P3c 已严格排除 0–5 E；证明 `OPT = 17` 仍需
> 排除 6–16 E。恢复本里程碑前，应先取得能够显著压缩搜索空间的新理论归约，
> 不再仅靠扩大当前逐层精确枚举。

P0 先建立小深度证明记录与参考重放闭环；只有状态等价、对称性和完备枚举机制
足够成熟后，才进入全局 16 E 下界计算。
...
- P0 没有应用反向 DAG 或 meet-in-the-middle，尚不能完成 6–16 E 的全局排除；
...
- 当前可以严格声明 profile 下 0–5 E 无解；这仍不接近 16 E 全局下界，17 E 仍只是已验证上界。

...
- 局部精确枚举找到一个已有点圆直接经过 Y，使步骤 (vi) 及后续圆心构造合计从 9 E 降为 6 E；
- 新证书由独立 SageMath 10.7 verifier 精确重放为 21 E：7 条直线、14 个圆；
- 两个相邻目标均在第 21 E 首次出现，无重复绘制；
- 相比同一 profile 的 32 E baseline，新的已验证上界降低 11 E；
- profile、证书、验证报告、机器可读依赖 DAG、Schema、来源台账、中文说明和回归测试均已发布；
- 该结果不包含 lower-bound proof，不声称 21 E 全局最优或文献最短。

...
> 状态：**待完成（当前暂停）**。现有正式结论为
> \(5 < \operatorname{OPT}\le 17\)。0–5 E 已被独立参考 checker 严格排除，
> 6–16 E 尚未穷尽。恢复工作前需要新的理论归约，而不是继续盲目扩宽逐层枚举。

本文定义 `regular-17-e-fixed-v1` 的下界证明路线。当前 17 E 证书给出

\[
\operatorname{OPT}\le 17.
\]

要证明 17 是真正最小值，还必须证明不存在分数不超过 16 E 的合法程序，即
...
当前实现已严格排除 0–5 E，但不证明 16 E 已被穷尽，也不改变当前“17 E 已验证
上界”的结论等级。
...
小深度；通向 18 E 的候选步骤依次为：

1. 把固定两步后缀推广为更深的精确反向 AND/OR 边界；
2. 让前向状态与反向边界按精确点/对象集合匹配；
3. 给并行分片增加确定性覆盖范围和可合并证明记录；
4. 把固定 E12/E16 局部窗口保留为历史校准，再评估全局 16 E；
5. 只有所有分支都由安全规则覆盖后，才允许发布 `OPT = 17`。

...
> 状态：**待完成（当前暂停）**。现有正式结论为
> \(5 < \operatorname{OPT}\le 12\)。0–5 E 已被独立参考 checker 严格排除，
> 6–11 E 尚未穷尽。恢复工作前需要新的理论归约，而不是继续盲目扩宽逐层枚举。

本文定义 `regular-17-e-fixed-v1` 的下界证明路线。当前 [QQ-GGB 12 E 证书](../qq-ggb/construction-12e-011.json)给出

\[
\operatorname{OPT}\le 12.
\]

要证明 12 是真正最小值，还必须证明不存在分数不超过 11 E 的合法程序，即
...
当前实现已严格排除 0–5 E，但尚未全局排除 6–11 E。QQ-GGB 的分支改写改善了构造上界；其[有限局部搜索](../qq-ggb/search-hour-2026-09-11.md)没有扩大 M7 的全局排除范围，12 E 仍未被证明最小。
...
`616a268ea02ed0785e5e055d157c298bc817484560de0f7998a3452d734a7463`，记录的 checker
...
以下工作保留为恢复 M7 后的研究路线，不在当前阶段继续执行。线性参考重放只适合
小深度；尝试扩大严格排除范围的候选步骤依次为：

1. 把固定两步后缀推广为更深的精确反向 AND/OR 边界；
2. 让前向状态与反向边界按精确点/对象集合匹配；
3. 给并行分片增加确定性覆盖范围和可合并证明记录；
4. 把历史 19 E 路线的固定 E12/E16 局部窗口保留为校准，再评估全局 11 E；
5. 只有所有分支都由安全规则覆盖后，才允许发布 `OPT = 12`。

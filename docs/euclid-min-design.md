...
形式模型、固定规则配置、精确几何内核、证书格式、Sage 验证器、文献构造转换、基础搜索器和 12 E 构造证书均已完成。当前还完成了 0–5 E 的有界穷尽证明记录。

现阶段的事实边界是：

- [QQ-GGB 12 E](../qq-ggb/README.md) 是通过项目验证器、独立根式检查和独立证书重放的构造上界；
- 32 E、19 E、17 E 及 QQ-GGB 首次 15 E 目标转换是同一规则下保留的历史结果；
- 0–5 E 已被严格排除，当前边界为 `5 < OPT ≤ 12`；
- 6–11 E 尚未完成穷尽，因此不能声称 12 E 全局最优；
- 系统文献检索尚未完成，QQ-GGB 的原作者、公开出处和分支改写优先权未确认，因此不能声称 12 E 是文献最短或世界纪录。

全局最优性工作暂时标记为待完成。恢复之前，应优先寻找能够显著压缩 6–11 E 状态空间的新理论归约，而不是继续无边界扩大启发式搜索。

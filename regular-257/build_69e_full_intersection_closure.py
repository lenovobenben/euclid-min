...
from full_intersection_closure import analyze_full_intersection_closure
...
OUTPUT_PATH = ROOT / "full-intersection-closure-69e.json"
...
def build_report(*, trace: bool = False) -> dict:
    ...
    if not verification["valid"]:
        raise ValueError("全交点闭包只能从验证通过的证书生成")
    ...
    return analyze_full_intersection_closure(
        certificate,
        source,
        trace=tracer,
    )
...
def main() -> None:
    ...
    print(
        f"unique_finite_real_points={summary['unique_finite_real_points']} "
        f"unnamed_points={summary['unnamed_points']}"
    )
    print(
    ...
    print(
        ...
        f"{result['minimum_required_paid_draws_within_fixed_object_universe']}"
    )

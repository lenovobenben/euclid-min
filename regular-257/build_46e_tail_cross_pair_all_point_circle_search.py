...
from tail_cross_pair_all_point_circle_search import search_all_point_cross_tail_circles
...
OUTPUT_PATH = ROOT / "tail-cross-pair-all-point-circle-search-46e.json"
...
def build_report(*, trace: bool = False) -> dict:
    ...
    return search_all_point_cross_tail_circles(
        certificate,
        ga_ir,
        source,
        trace=tracer,
...
def main() -> None:
    ...
    print(
        f"center_checks={summary['strict_ball_center_checks']} "
        f"survivors={summary['strict_ball_survivors']} "
        f"center_incidences={summary['center_incidences']}"
    )
    print(
        f"existing={summary['existing_circle_center_incidences']} "
        f"new_exact={summary['new_exact_center_incidences']} "
        f"new_abstract={summary['new_abstract_center_incidences']}"
    )
